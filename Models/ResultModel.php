...
class ResultModel extends BaseModel
{
    ...
    public function getUserResultListModel($id)
    {
        ...
        try {
            $query = $this->conn->prepare("SELECT results.id,results.id_user,results.id_exam,results.score,results.duration,results.created_at,exams.title from results INNER JOIN exams ON results.id_exam = exams.id  WHERE results.id_user=:id_user ORDER BY results.id DESC LIMIT :limit OFFSET :offset ");
            $query->bindParam(':limit', $limit, PDO::PARAM_INT);
            $query->bindParam(':offset', $offset, PDO::PARAM_INT);
    ...
    public function getReviewModel($id)
    {
        ...
        try {
            $query = $this->conn->prepare("select id_question,answer,state from $this->tableResultDetail where id_results=:id_results order by id_question");
    ...
    public function createResultExam($data)
    {
        ...
        try {
            ...
            foreach ($listQuestionInExam as $row) {
                ...
                $queryGetCorrectAns = $this->conn->prepare("select id,correctAns from questions where id=:id");
            ...
            $query2 = $this->conn->prepare("select count_user_do from exams where id=:id");
            ...
            $query6 = $this->conn->prepare("update exams set count_user_do=:number where id=:id");
            $query6->execute(['number' => $count_user_do + 1, 'id' => $idExam]);
            // thêm dữ liệu vào bảng result_question
            $query3 = $this->conn->prepare("insert into result_detail set id_results=:id_results,id_question=:id_question,answer=:answer,state=:state");
            if ($answer !== null) {
            ...
            foreach ($listQuestionInExam as $row3) {
                $idQuestion = $row3['id'];
                $query5 = $this->conn->prepare("select id_question from result_detail where id_question =:id");
                ...
                if ($resultIdQues) {
                    ...
                    $query4 = $this->conn->prepare("SELECT result_detail.number_do_wrong from result_detail inner join results on result_detail.id_results = results.id where result_detail.id_question=:id and results.id_exam =:id_exam");
                    $query4->execute(['id' => $idQuestion, 'id_exam' => $idExam]);
                    $result = $query4->fetch();
                    $numberWrong = $result->number_do_wrong;
                    // cập nhật số người làm sai 
                    $query5 = $this->conn->prepare("UPDATE result_detail inner join results on result_detail.id_results = results.id set result_detail.number_do_wrong=:number where result_detail.id_question=:id and results.id_exam =:id_exam ");
